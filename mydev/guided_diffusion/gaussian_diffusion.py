...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, cfg, dataset, model_kwargs=None, noise=None):
        """
        Compute training losses for a single timestep.

        :param model: the model to evaluate loss on.
        :param x_start: the [B x L x 3] tensor of inputs.
        :param t: a batch [B] of timestep indices.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    res = th.from_numpy(arr).to(device=timesteps.device)[timesteps].to(timesteps.device)
